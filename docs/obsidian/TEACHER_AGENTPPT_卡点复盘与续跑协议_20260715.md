...
- 无图片主体链的真人按钮验收已经通过 22/22；
- 当前卡点收敛为远程逐页图片的异步执行、持久化、断点恢复、版本同源和渲染验收；
- 下一步不是重新设计版面或重复跑整套同步脚本，而是先建立可恢复的图片任务状态机。
...
| 单页图片需 1–2 分钟 | 任务是否异步化、页面是否落盘 | 继续提高整套请求超时 | 创建任务记录，受控并发，逐页完成即持久化 |
| HTTP 200 但图片来源不明 | provider、requestId、图片字节和版本绑定 | 把 fallback 写成真实成功 | 明确返回 remote_success/fallback/failed |
| 中途失败后重新生成全部页面 | 已完成页面是否有稳定资产 ID | 从头运行整套 E2E | 只重试失败页并复用已完成资产 |
| PPTX 能解析但视觉未知 | 是否完成逐页渲染和溢出检查 | 只报页数和文件大小 | 渲染 PNG，检查空白、溢出、图片版本一致性 |
...
在现有 22/22 主体链之上实现图片生成任务状态机：任务异步提交，页面级状态与资产即时持久化，最多 2 路受控并发，失败只重试当前页，并显式记录 `remote_success`、`fallback`、`failed`。完成后再运行火山/品川独立验收以及浏览器/PPTX 同源和逐页渲染验收；不能跳去重新设计版面，也不能用本地 9 页替代产物宣告闭环。
