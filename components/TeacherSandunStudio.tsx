...
function TeacherScorePanel({ report }: { report?: TeacherDeckScoreReportV3 }) {
  ...
  return <div className="mt-4 border border-[#dfe5ee] p-4"><div className="flex items-center justify-between"><div className="text-xs font-semibold text-[#475467]">课堂就绪评分 V3</div><div className="text-lg font-bold text-[#171719]">{report.scores.total}<span className="ml-1 text-[10px] font-normal text-[#667085]">/100</span></div></div><div className="mt-3 grid grid-cols-2 gap-2 text-[11px] text-[#667085]"><div>教材对齐 <b className="float-right text-[#344054]">{report.scores.textbookAlignment}/25</b></div><div>教学设计 <b className="float-right text-[#344054]">{report.scores.pedagogy}/25</b></div><div>学科正确 <b className="float-right text-[#344054]">{report.scores.subjectCorrectness}/20</b></div><div>视觉表达 <b className="float-right text-[#344054]">{report.scores.visual}/15</b></div><div>工程质量 <b className="float-right text-[#344054]">{report.scores.engineering}/10</b></div><div>教师验证 <b className="float-right text-[#344054]">{report.scores.teacherEfficiency}/5</b></div></div><div className="mt-3 border-t border-[#eef1f6] pt-3 text-[11px] leading-5 text-[#667085]">证据覆盖 {report.evidence.averageCoverage}% · 可追溯来源 {report.evidence.traceableSources} · P0 {report.p0.length}</div>{report.p0.length ? <div className="mt-2 space-y-1">{report.p0.slice(0, 3).map((issue) => <div key={issue} className="bg-[#fff1f3] px-2 py-1.5 text-[11px] leading-4 text-[#b42318]">{issue}</div>)}</div> : <div className="mt-2 bg-[#ecfdf3] px-2 py-1.5 text-[11px] text-[#027a48]">教材与课堂硬门槛未发现 P0</div>}</div>;
}

export function TeacherSandunStudio({ project, workspaceIdentity, isExporting, isRefining, isGeneratingVisuals, isApplyingReviewFixes, isPresenting, generatedVisuals, onExport, onPresent, onClosePresent, onProjectChange, onRefine, onManualSave, isSavingSlide, teacherExportMeta, onGenerateVisuals, visualGenerationProgress, onApplyReviewFixes, onApplyPageReviewFixes, teacherVersions, teacherMaterials, isViewingCurrentVersion, onSelectVersion, onRestoreVersion, isRestoringVersion, teacherChat, isChatSending, onSendChat, isApplyingChatSuggestion, onApplyChatSuggestion, isAttachingMaterial, onAttachMaterial, isSubmittingReview, onSubmitReview }: TeacherSandunStudioProps) {
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <aside className={cn("absolute bottom-0 right-0 top-16 z-30 flex w-[min(380px,92vw)] flex-col border-l border-[#e0e6ef] bg-white shadow-[-16px_0_40px_rgba(20,32,52,0.12)] transition-transform xl:static xl:top-auto xl:w-[350px] xl:shrink-0 xl:translate-x-0 xl:shadow-none", toolsOpen ? "translate-x-0" : "translate-x-full")}>
          ...
          <div className="thin-scrollbar min-h-0 flex-1 overflow-y-auto p-4">
            {tool === "check" ? <TeacherScorePanel report={project.reviewCenter?.teacherScoreV3} /> : null}
