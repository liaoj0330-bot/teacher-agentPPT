...
type TeacherSandunStudioProps = {
  ...
  onGenerateVisuals: (options?: { retryOnlyFailed?: boolean }) => void;
  visualGenerationProgress?: VisualGenerationProgress | null;
...
function Thumbnail({ index, slide, scene, active, onClick }: { index: number; slide: DesignSlide; scene?: RenderScene; active: boolean; onClick: () => void }) {
  return <button type="button" onClick={onClick} className={cn("w-full border bg-white p-2 text-left transition", active ? "border-[#2f7cff] shadow-[0_6px_18px_rgba(47,124,255,0.12)]" : "border-[#e0e6ef] hover:border-[#a8c9ff]")}><div className="aspect-video overflow-hidden bg-[#edf4ff]">{scene ? <BrowserSceneRenderer scene={scene} /> : <div className="flex h-full flex-col justify-between border border-white/80 bg-white/75 p-2"><span className="text-[10px] font-bold text-[#2f7cff]">{String(index + 1).padStart(2, "0")}</span><span className="line-clamp-2 text-[10px] font-semibold leading-4 text-ink">{slide.title}</span></div>}</div><span className="mt-2 block line-clamp-1 text-[11px] text-[#667085]">{slide.title}</span></button>;
}

function CompositionSummary({ scene }: { scene?: RenderScene }) {
  if (!scene?.composition) return null;
  const composition = scene.composition;
  return <dl className="mt-4 grid grid-cols-2 gap-x-3 gap-y-2 border-y border-[#e7ebf1] py-3 text-[11px]"><div><dt className="text-[#667085]">构图</dt><dd className="mt-0.5 font-semibold text-[#344054]">{composition.label}</dd></div><div><dt className="text-[#667085]">密度</dt><dd className="mt-0.5 font-semibold text-[#344054]">{composition.densityLevel} · {composition.maxBlocks} 块</dd></div><div><dt className="text-[#667085]">字号预算</dt><dd className="mt-0.5 font-semibold text-[#344054]">标题 {composition.titlePt} / 正文 ≥ {composition.bodyMinPt} pt</dd></div><div><dt className="text-[#667085]">内容预算</dt><dd className="mt-0.5 font-semibold text-[#344054]">≤ {composition.maxCharacters} 字</dd></div></dl>;
}

function TeacherScorePanel({ report }: { report?: TeacherDeckScoreReportV3 }) {
  if (!report) return null;
  const automaticScore = report.scores.total - report.scores.teacherEfficiency;
  return <div className="mt-4 border border-[#dfe5ee] p-4"><div className="flex items-center justify-between"><div><div className="text-xs font-semibold text-[#475467]">课前自动检查 V3</div><div className="mt-1 text-[10px] text-[#667085]">机器证据分，不代替真实教师试讲</div></div><div className="text-lg font-bold text-[#171719]">{automaticScore}<span className="ml-1 text-[10px] font-normal text-[#667085]">/95</span></div></div><div className="mt-3 grid grid-cols-2 gap-2 text-[11px] text-[#667085]"><div>教材对齐 <b className="float-right text-[#344054]">{report.scores.textbookAlignment}/25</b></div><div>教学设计 <b className="float-right text-[#344054]">{report.scores.pedagogy}/25</b></div><div>学科正确 <b className="float-right text-[#344054]">{report.scores.subjectCorrectness}/20</b></div><div>视觉表达 <b className="float-right text-[#344054]">{report.scores.visual}/15</b></div><div>工程质量 <b className="float-right text-[#344054]">{report.scores.engineering}/10</b></div><div>真实试讲证据 <b className="float-right text-[#344054]">{report.scores.teacherEfficiency}/5</b></div></div><div className="mt-3 bg-[#fff8e8] px-2 py-2 text-[10px] leading-4 text-[#7a4d00]">{report.requiresTeacherConfirmation ? "真实试讲证据尚未完整记录；提交审核本身不计试讲分。" : "已记录结构化真实试讲证据；商业发布仍以外部门禁为准。"}</div><div className="mt-3 border-t border-[#eef1f6] pt-3 text-[11px] leading-5 text-[#667085]">证据覆盖 {report.evidence.averageCoverage}% · 可追溯来源 {report.evidence.traceableSources} · P0 {report.p0.length}</div>{report.p0.length ? <div className="mt-2 space-y-1">{report.p0.slice(0, 3).map((issue) => <div key={issue} className="bg-[#fff1f3] px-2 py-1.5 text-[11px] leading-4 text-[#b42318]">{issue}</div>)}</div> : <div className="mt-2 bg-[#ecfdf3] px-2 py-1.5 text-[11px] text-[#027a48]">教材与课堂硬门槛未发现 P0</div>}</div>;
}

const trialRubricLabels: Record<TeacherTrialRubricKey, string> = {
  goalAchievement: "目标达成",
  pacing: "课堂节奏",
  interaction: "师生互动",
  practiceFeedback: "练习反馈",
  teacherNotesUsability: "讲稿可用",
};
...
function TeacherTrialEvidencePanel({ validation, isSubmitting, isViewingCurrentVersion, plannedDurationMinutes, onSubmit }: {
  validation?: TeacherTrialValidation;
  isSubmitting: boolean;
  isViewingCurrentVersion: boolean;
  plannedDurationMinutes: number;
  onSubmit: (evidence: TeacherTrialEvidenceInput) => void;
}) {
  const [draft, setDraft] = useState<TrialDraft>({ trialStartedAt: "", trialEndedAt: "", plannedDurationMinutes: String(plannedDurationMinutes || 45), actualDurationMinutes: "", classSize: "", software: "", device: "", rubric: {}, issues: "", reuseDecision: "", teacherComment: "" });
  const derivedDuration = draft.trialStartedAt && draft.trialEndedAt ? Math.round((Date.parse(draft.trialEndedAt) - Date.parse(draft.trialStartedAt)) / 60_000) : 0;
  const actualDuration = Number(draft.actualDurationMinutes) || derivedDuration;
  const clientErrors = [
    !draft.trialStartedAt || !draft.trialEndedAt ? "请填写试讲开始和结束时间" : "",
    draft.trialStartedAt && draft.trialEndedAt && derivedDuration <= 0 ? "结束时间必须晚于开始时间" : "",
    Number(draft.plannedDurationMinutes) <= 0 ? "请填写计划时长" : "",
    actualDuration <= 0 ? "请填写实际时长，或用起止时间自动计算" : "",
    Number(draft.classSize) <= 0 ? "请填写班级人数" : "",
    !draft.software.trim() ? "请填写授课软件" : "",
    !draft.device.trim() ? "请填写授课设备" : "",
    teacherTrialRubricKeys.some((key) => !draft.rubric[key]) ? "请完成五项课堂量表" : "",
    !draft.reuseDecision ? "请选择复用结论" : "",
  ].filter(Boolean);
  const status = validation?.status || "pending";
  const statusLabel = status === "complete" ? "真实试讲证据完整" : status === "invalid" ? "真实试讲证据无效" : "尚未记录真实试讲";
  const statusTone = status === "complete" ? "border-[#abefc6] bg-[#ecfdf3] text-[#027a48]" : status === "invalid" ? "border-[#fecdca] bg-[#fff1f3] text-[#b42318]" : "border-[#fed7aa] bg-[#fffaf0] text-[#92400e]";
  const update = <K extends keyof TrialDraft>(key: K, value: TrialDraft[K]) => setDraft((current) => ({ ...current, [key]: value }));
  const submit = () => {
    if (clientErrors.length) return;
    onSubmit({ trialStartedAt: new Date(draft.trialStartedAt).toISOString(), trialEndedAt: new Date(draft.trialEndedAt).toISOString(), plannedDurationMinutes: Number(draft.plannedDurationMinutes), actualDurationMinutes: actualDuration, classSize: Number(draft.classSize), software: draft.software, device: draft.device, rubric: draft.rubric, issues: draft.issues.split(/\r?\n/).map((item) => item.trim()).filter(Boolean), reuseDecision: draft.reuseDecision || undefined, teacherComment: draft.teacherComment });
  };
  return <section data-testid="teacher-trial-evidence" className="mt-4 border border-[#dfe5ee] p-4">
    <div className="flex items-start justify-between gap-3"><div><h3 className="text-xs font-semibold text-[#475467]">真实课堂试讲</h3><p className="mt-1 text-[10px] leading-4 text-[#667085]">此证据由教师填写，与课前自动评分分开记录。</p></div><span data-testid="teacher-trial-status" className={cn("shrink-0 border px-2 py-1 text-[10px] font-semibold", statusTone)}>{statusLabel}</span></div>
    {status === "complete" ? <dl className="mt-3 grid grid-cols-2 gap-2 border-t border-[#eef1f6] pt-3 text-[11px] text-[#667085]"><div>量表均分 <b className="float-right text-[#344054]">{validation?.rubricAverage ?? "-"}/5</b></div><div>实际时长 <b className="float-right text-[#344054]">{validation?.actualDurationMinutes ?? "-"} 分钟</b></div></dl> : null}
    {validation?.errors.length ? <div className="mt-2 text-[11px] leading-5 text-[#b42318]">{validation.errors.join("；")}</div> : null}
    <div className="mt-4 grid grid-cols-2 gap-2">
      <label className="text-[11px] font-semibold text-[#475467]">开始时间<input aria-label="试讲开始时间" type="datetime-local" value={draft.trialStartedAt} onChange={(event) => update("trialStartedAt", event.target.value)} className="mt-1 h-9 w-full border border-[#dfe5ee] px-2 text-xs" /></label>
      <label className="text-[11px] font-semibold text-[#475467]">结束时间<input aria-label="试讲结束时间" type="datetime-local" value={draft.trialEndedAt} onChange={(event) => update("trialEndedAt", event.target.value)} className="mt-1 h-9 w-full border border-[#dfe5ee] px-2 text-xs" /></label>
      <label className="text-[11px] font-semibold text-[#475467]">计划时长<input aria-label="计划时长（分钟）" type="number" min="1" value={draft.plannedDurationMinutes} onChange={(event) => update("plannedDurationMinutes", event.target.value)} className="mt-1 h-9 w-full border border-[#dfe5ee] px-2 text-xs" /></label>
      <label className="text-[11px] font-semibold text-[#475467]">实际时长<input aria-label="实际时长（分钟）" type="number" min="1" placeholder={derivedDuration > 0 ? `自动 ${derivedDuration}` : "分钟"} value={draft.actualDurationMinutes} onChange={(event) => update("actualDurationMinutes", event.target.value)} className="mt-1 h-9 w-full border border-[#dfe5ee] px-2 text-xs" /></label>
      <label className="text-[11px] font-semibold text-[#475467]">班级人数<input aria-label="班级人数" type="number" min="1" value={draft.classSize} onChange={(event) => update("classSize", event.target.value)} className="mt-1 h-9 w-full border border-[#dfe5ee] px-2 text-xs" /></label>
      <label className="text-[11px] font-semibold text-[#475467]">授课软件<input aria-label="授课软件" value={draft.software} onChange={(event) => update("software", event.target.value)} placeholder="WPS / PowerPoint" className="mt-1 h-9 w-full border border-[#dfe5ee] px-2 text-xs" /></label>
      <label className="col-span-2 text-[11px] font-semibold text-[#475467]">授课设备<input aria-label="授课设备" value={draft.device} onChange={(event) => update("device", event.target.value)} placeholder="教室电脑、投影或一体机" className="mt-1 h-9 w-full border border-[#dfe5ee] px-2 text-xs" /></label>
    </div>
    <div className="mt-4 space-y-2">{teacherTrialRubricKeys.map((key) => <div key={key} className="flex items-center justify-between gap-2 text-[11px]"><span className="font-semibold text-[#475467]">{trialRubricLabels[key]}</span><div className="flex" role="group" aria-label={trialRubricLabels[key]}>{[1, 2, 3, 4, 5].map((score) => <button key={score} type="button" aria-label={`${trialRubricLabels[key]} ${score}分`} aria-pressed={draft.rubric[key] === score} onClick={() => update("rubric", { ...draft.rubric, [key]: score })} className={cn("flex size-7 items-center justify-center border-y border-r text-[11px] first:border-l", draft.rubric[key] === score ? "border-[#2f7cff] bg-[#2f7cff] text-white" : "border-[#dfe5ee] bg-white text-[#667085]")}>{score}</button>)}</div></div>)}</div>
    <label className="mt-4 block text-[11px] font-semibold text-[#475467]">复用结论<select aria-label="复用结论" value={draft.reuseDecision} onChange={(event) => update("reuseDecision", event.target.value as TrialDraft["reuseDecision"])} className="mt-1 h-9 w-full border border-[#dfe5ee] bg-white px-2 text-xs"><option value="">请选择</option><option value="as_is">可直接复用</option><option value="after_revision">修改后复用</option><option value="reject">不建议复用</option></select></label>
    <label className="mt-3 block text-[11px] font-semibold text-[#475467]">课堂问题（每行一项）<textarea aria-label="课堂问题" value={draft.issues} onChange={(event) => update("issues", event.target.value)} className="mt-1 min-h-16 w-full border border-[#dfe5ee] p-2 text-xs leading-5" /></label>
    <label className="mt-3 block text-[11px] font-semibold text-[#475467]">教师备注<textarea aria-label="教师备注" value={draft.teacherComment} onChange={(event) => update("teacherComment", event.target.value)} className="mt-1 min-h-16 w-full border border-[#dfe5ee] p-2 text-xs leading-5" /></label>
    {clientErrors.length ? <div data-testid="teacher-trial-client-errors" className="mt-3 text-[11px] leading-5 text-[#92400e]">待完成：{clientErrors.join("；")}</div> : null}
    <button type="button" onClick={submit} disabled={isSubmitting || !isViewingCurrentVersion || clientErrors.length > 0} className="mt-3 flex h-10 w-full items-center justify-center gap-2 bg-[#171719] text-sm font-semibold text-white disabled:opacity-50">{isSubmitting ? <Loader2 className="size-4 animate-spin" /> : <ClipboardCheck className="size-4" />}保存试讲证据并提交审核</button>
  </section>;
}
...
export function TeacherSandunStudio({ project, workspaceIdentity, isExporting, isRefining, isGeneratingVisuals, isApplyingReviewFixes, isPresenting, generatedVisuals, onExport, onPresent, onClosePresent, onProjectChange, onRefine, onManualSave, isSavingSlide, teacherExportMeta, onGenerateVisuals, visualGenerationProgress, onApplyReviewFixes, onApplyPageReviewFixes, teacherVersions, teacherMaterials, isViewingCurrentVersion, onSelectVersion, onRestoreVersion, isRestoringVersion, teacherChat, isChatSending, onSendChat, isApplyingChatSuggestion, onApplyChatSuggestion, isAttachingMaterial, onAttachMaterial, isSubmittingReview, teacherTrialValidation, onSubmitReview, betaOperations = defaultTeacherBetaOperations, onOpenFeedback }: TeacherSandunStudioProps) {
  ...
  const previewTruth = useMemo(() => {
    if (!project.deckSpec) return null;
    try {
  ...
  }, [generatedVisuals, project.deckSpec, project.slides]);
  const activeScene = previewTruth?.scenes.find((scene) => scene.slideId === activeSlide?.id) || previewTruth?.scenes[activeIndex];
  ...
  const deliveryPack = project.contentPlan?.deliveryPack;
  const activeTeacherNote = deliveryPack?.teacherNotes.find((note) => note.pageId === activeSlide?.id)
    || deliveryPack?.teacherNotes[activeIndex];
  ...
  const tabs: Array<{ id: ToolTab; label: string; icon: typeof FileText }> = [
    { id: "content", label: "内容", icon: FileText }, { id: "delivery", label: "交付包", icon: BookOpen }, { id: "design", label: "设计", icon: LayoutTemplate }, { id: "materials", label: "教材", icon: ImagePlus }, { id: "classroom", label: "课堂", icon: MessageSquareText }, { id: "chat", label: "对话", icon: MessageCircle }, { id: "check", label: "检查", icon: ClipboardCheck }
  ];
  ...
  <main className="flex h-dvh overflow-hidden bg-[#f3f5f9] text-ink">
    <section className="flex min-w-0 flex-1 flex-col">
      ...
      {!isViewingCurrentVersion ? <div className="flex shrink-0 items-center justify-between gap-3 border-b border-[#fed7aa] bg-[#fffaf0] px-4 py-2 text-xs text-[#92400e]"><span className="flex items-center gap-2"><AlertTriangle className="size-4" />正在查看历史版本，为只读状态。可以返回当前版本，或将此快照恢复为新版本。</span><span className="flex shrink-0 items-center gap-2">{workspaceIdentity?.versionId ? <button type="button" onClick={() => onRestoreVersion(workspaceIdentity.versionId)} disabled={isRestoringVersion} className="flex h-7 items-center gap-1.5 border border-[#f0ba7a] bg-[#92400e] px-2.5 font-semibold text-white disabled:opacity-50">{isRestoringVersion ? <Loader2 className="size-3.5 animate-spin" /> : <History className="size-3.5" />}恢复为新版本</button> : null}{currentVersion ? <button type="button" onClick={() => onSelectVersion(currentVersion.versionId)} className="flex h-7 items-center gap-1.5 border border-[#f0ba7a] bg-white px-2.5 font-semibold text-[#92400e] hover:bg-[#fff4e6]"><RotateCcw className="size-3.5" />返回当前版本</button> : null}</span></div> : null}
      <div className="flex min-h-0 flex-1">
        <aside className="thin-scrollbar hidden w-[208px] shrink-0 overflow-y-auto border-r border-[#e0e6ef] bg-white px-3 py-4 md:block"><div className="flex items-center justify-between px-1 text-xs font-semibold text-[#475467]"><span>课程结构</span><span>{project.slides.length} 页</span></div><div className="mt-4 space-y-5">{sections.map((group) => <section key={`${group.label}-${group.pages[0]?.index ?? 0}` }><div className="mb-2 flex items-center gap-2 px-1 text-[11px] font-semibold text-[#667085]"><ChevronDown className="size-3.5" />{group.label}</div><div className="space-y-2">{group.pages.map(({ slide, index }) => <Thumbnail key={slide.id || index} index={index} slide={slide} scene={previewTruth?.scenes.find((scene) => scene.slideId === slide.id) || previewTruth?.scenes[index]} active={index === activeIndex} onClick={() => setActiveIndex(index)} />)}</div></section>)}</div></aside>
        ...
        <aside className={cn("absolute bottom-0 right-0 top-16 z-30 flex w-[min(380px,92vw)] flex-col border-l border-[#e0e6ef] bg-white shadow-[-16px_0_40px_rgba(20,32,52,0.12)] transition-transform xl:static xl:top-auto xl:w-[350px] xl:shrink-0 xl:translate-x-0 xl:shadow-none", toolsOpen ? "translate-x-0" : "translate-x-full")}>
          ...
          <div className="thin-scrollbar min-h-0 flex-1 overflow-y-auto p-4">
            {tool === "check" ? <><TeacherScorePanel report={project.reviewCenter?.teacherScoreV3} /><TeacherTrialEvidencePanel validation={teacherTrialValidation} isSubmitting={isSubmittingReview} isViewingCurrentVersion={isViewingCurrentVersion} plannedDurationMinutes={project.contentPlan?.lessonPlan?.totalMinutes || project.contentPlan?.lessonBlueprint?.lessonPlan?.totalMinutes || 45} onSubmit={(evidence) => onSubmitReview(evidence)} /></> : null}
            ...
            {tool === "delivery" ? <div data-testid="teacher-delivery-pack"><div className="flex items-start justify-between gap-3"><div><h2 className="text-sm font-semibold">教师交付包</h2><p className="mt-1 text-xs leading-5 text-muted">与当前课件版本同步的讲稿、答案、板书和作业。</p></div><span className="shrink-0 bg-[#ecfdf3] px-2 py-1 text-[10px] font-semibold text-[#027a48]">{deliveryPack?.readiness === "ready_for_teacher_review" ? "待教师确认" : "需复核"}</span></div>{deliveryPack ? <><div className="mt-4 grid grid-cols-2 border border-[#dfe5ee] text-center text-[11px] text-[#667085]"><div className="border-b border-r border-[#dfe5ee] p-3"><b className="block text-lg text-[#171719]">{deliveryPack.teacherNotes.length}</b>逐页讲稿</div><div className="border-b border-[#dfe5ee] p-3"><b className="block text-lg text-[#171719]">{deliveryPack.answerKey.length}</b>答案要点</div><div className="border-r border-[#dfe5ee] p-3"><b className="block text-lg text-[#171719]">{deliveryPack.boardPlan.columns.length}</b>板书分区</div><div className="p-3"><b className="block text-lg text-[#171719]">{deliveryPack.homework.length}</b>分层作业</div></div><section className="mt-5 border-t border-[#e7ebf1] pt-4"><div className="flex items-center justify-between"><h3 className="text-xs font-semibold text-[#475467]">本页教师讲稿</h3><span className="text-[11px] text-muted">第 {activeIndex + 1} 页</span></div>{activeTeacherNote ? <div className="mt-2 space-y-2 text-xs leading-5 text-[#344054]"><b className="block text-sm text-[#171719]">{activeTeacherNote.title} · {activeTeacherNote.durationMinutes} 分钟</b><p>{activeTeacherNote.teacherAction}</p><p className="border-l-2 border-[#2f7cff] pl-3 text-[#175cd3]">提问：{activeTeacherNote.prompt}</p><p className="text-[#667085]">预期：{activeTeacherNote.expectedResponse}</p><p className="text-[#667085]">备用：{activeTeacherNote.fallbackAction}</p></div> : <p className="mt-2 text-xs text-muted">当前页没有关联讲稿。</p>}</section><section className="mt-5 border-t border-[#e7ebf1] pt-4"><h3 className="text-xs font-semibold text-[#475467]">答案与评价标准</h3><div className="mt-2 space-y-3">{deliveryPack.answerKey.map((item, index) => <div key={`${item.eventId}-${index}`} className="border-l-2 border-[#98a2b3] pl-3 text-xs leading-5 text-[#344054]"><b className="block">答案 {index + 1}</b><p>{item.answer}</p><p className="text-[#667085]">评分：{item.scoringCriteria}</p></div>)}</div></section><section className="mt-5 border-t border-[#e7ebf1] pt-4"><h3 className="text-xs font-semibold text-[#475467]">{deliveryPack.boardPlan.title}</h3><div className="mt-2 space-y-3">{deliveryPack.boardPlan.columns.map((column) => <div key={column.heading} className="text-xs leading-5"><b className="text-[#171719]">{column.heading}</b><ul className="mt-1 list-disc pl-4 text-[#667085]">{column.items.map((item) => <li key={item}>{item}</li>)}</ul></div>)}</div></section><section className="mt-5 border-t border-[#e7ebf1] pt-4"><h3 className="text-xs font-semibold text-[#475467]">分层作业</h3><div className="mt-2 space-y-3">{deliveryPack.homework.map((item) => <div key={item.level} className="text-xs leading-5"><b className="text-[#171719]">{item.level}</b><p className="text-[#344054]">{item.task}</p><p className="text-[#667085]">达成：{item.successCriteria}</p></div>)}</div></section></> : <div className="mt-4 border border-dashed border-[#ccd7e6] p-4 text-xs leading-5 text-muted">当前版本没有完整教师交付包，请返回课堂方案重新生成。</div>}</div> : null}
            {tool === "design" ? <div><h2 className="text-sm font-semibold">页面设计</h2><p className="mt-1 text-xs leading-5 text-muted">调整本页版式，或用 Sandun 补充关键视觉。</p><CompositionSummary scene={activeScene} /><label className="mt-5 block text-xs font-semibold text-[#475467]">页面版式<select value={activeSlide?.layout || "cards"} onChange={(event) => patchActive({ layout: event.target.value as SlideLayout })} className="mt-2 h-10 w-full border border-[#dfe5ee] bg-white px-3 text-sm focus:border-[#2f7cff] focus:ring-[#2f7cff]/10">{layouts.map((layout) => <option key={layout} value={layout}>{layout}</option>)}</select></label><button type="button" onClick={() => activeSlide?.id && onManualSave(activeSlide.id, { layout: activeSlide.layout })} disabled={isSavingSlide || !isViewingCurrentVersion || !activeSlide?.id} className="mt-3 flex h-10 w-full items-center justify-center gap-2 border border-[#bed5ff] bg-[#f3f8ff] text-sm font-semibold text-[#175cd3] disabled:opacity-50">{isSavingSlide ? <Loader2 className="size-4 animate-spin" /> : <Save className="size-4" />}保存本页版式</button><div className="mt-5 border border-[#dfe5ee] bg-[#f8fafc] p-4"><div className="flex items-center gap-2 text-sm font-semibold"><ImagePlus className="size-4 text-[#2f7cff]" />配图与版式</div><p className="mt-2 text-xs leading-5 text-[#667085]">为封面和关键页生成视觉内容，保持整套课件的层级和节奏。</p><button type="button" onClick={() => onGenerateVisuals()} disabled={isGeneratingVisuals} className="mt-4 flex h-10 w-full items-center justify-center gap-2 bg-[#2f7cff] text-sm font-semibold text-white disabled:opacity-50">{isGeneratingVisuals ? <Loader2 className="size-4 animate-spin" /> : <ImagePlus className="size-4" />}生成页面视觉</button>{visualGenerationProgress ? <div className="mt-3 space-y-2" aria-live="polite"><div className="flex items-center justify-between text-[11px] text-[#667085]"><span>{visualGenerationProgress.active ? "正在生成视觉" : "视觉生成完成"}</span><span>{visualGenerationProgress.completed}/{visualGenerationProgress.total}</span></div><div className="h-1.5 overflow-hidden bg-[#e5e7eb]"><div className="h-full bg-[#2f7cff] transition-all" style={{ width: `${visualGenerationProgress.total ? (visualGenerationProgress.completed / visualGenerationProgress.total) * 100 : 0}%` }} /></div>{visualGenerationProgress.failed ? <button type="button" onClick={() => onGenerateVisuals({ retryOnlyFailed: true })} disabled={isGeneratingVisuals} className="flex h-8 w-full items-center justify-center gap-1.5 border border-[#fca5a5] bg-[#fff1f2] text-xs font-semibold text-[#b42318]"><RotateCcw className="size-3.5" />重试失败页（{visualGenerationProgress.failed}）</button> : null}</div> : null}</div><button type="button" onClick={() => onRefine("请统一整套课件的标题层级、留白和视觉节奏，保留教学内容。", "deck") } disabled={isRefining} className="mt-3 flex h-10 w-full items-center justify-center gap-2 border border-[#dfe5ee] text-sm font-semibold text-[#344054] disabled:opacity-50"><RefreshCw className="size-4" />统一整套版式</button></div> : null}
