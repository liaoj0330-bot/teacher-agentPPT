...
function addText(slide: pptxgen.Slide, text: string, options: pptxgen.TextPropsOptions) {
  slide.addText(text || "", {
    fontFace: options.fontFace || (options.bold ? PPTX_HEAD_FONT : PPTX_BODY_FONT),
    breakLine: false,
    fit: "shrink",
    margin: 0,
    ...options
...
async function renderDeckToBuffer(
  ...
  pptx.theme = { headFontFace: PPTX_HEAD_FONT, bodyFontFace: PPTX_BODY_FONT };
...
async function handleVersionedExport(body: {
  ...
  if (artifactType === "pptx" && taskContext.generationMode === "optimize_existing" && taskContext.beautifyOptions?.intensity === "preserve") {
    ...
    const persisted = await persistCoursewareArtifactBuffer({
      userId: user.id,
      projectId: source.projectId,
      versionId: source.versionId,
      artifactType: "pptx",
